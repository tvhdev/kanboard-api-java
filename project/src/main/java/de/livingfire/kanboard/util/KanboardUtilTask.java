...
public class KanboardUtilTask extends KanboardUtil<KanboardTask> {
    ...
    @Override
    public Map<String, Object> convertToMap(KanboardTask task) {
        if (task == null) {
            return null;
        }
    ...
    @Override
    public KanboardTask convertToObject(Map<String, Object> h) {
        if (h == null) {
            return null;
        }
        KanboardTask task = new KanboardTask();
        task.setId(String.valueOf(h.get(PARAM_ID)));
        task.setTitle((String)h.get(PARAM_TITLE));
        task.setDescription((String)h.get(PARAM_DESCRIPTION));
        ...
        task.setDateCompleted((String)h.get(PARAM_DATE_COMPLETED));
        ...
        task.setReference((String)h.get(PARAM_REFERENCE));
        task.setDateStarted(String.valueOf(h.get(PARAM_DATE_STARTED)));
        task.setTimeSpent((String)h.get(PARAM_TIME_SPEND));
        task.setTimeEstimated((String)h.get(PARAM_TIME_ESTIMATED));
